...
Implement automatic project stage progression based on quest completion. Move projects through IDEA → PROTOTYPE → BUILD → PROJECT stages.
...
export function StageIndicator({ stage, questsCompleted = 0 }) {
  ...
  return (
    <div className="space-y-4">
        ...
        {stages.map((stageName, index) => (
          <div key={stageName} className="flex items-center">
            ...
            <div className="ml-2 text-sm">
              <div className={cn('font-medium', index === currentIndex && 'text-primary')}>
                {stageName}
              </div>
              <div className="text-foreground text-xs">{PROJECT_STAGES[stageName].description}</div>
            </div>

            {index < stages.length - 1 && (
              <div
                className={cn('mx-4 h-px flex-1', index < currentIndex ? 'bg-primary' : 'bg-muted')}
              />
            )}
          </div>
        ))}
      ...
      <StageProgress stage={stage} questsCompleted={questsCompleted} />
    </div>
  )
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        ...
        <span>
          {questsCompleted}/{stageInfo.requiredQuests} quests
        </span>
      </div>
