  ...
  return (
    <div className="container py-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Participate in multiple programs to access more quests
          </p>
        </div>
      ...
      <section className="mb-8">
        <h2 className="text-2xl font-semibold mb-4">Active Programs</h2>
      ...
      {completedPrograms.length > 0 && (
        <section>
          <h2 className="text-2xl font-semibold mb-4">Completed Programs</h2>
          <div className="grid gap-4 md:grid-cols-2">
...
export function ProgramParticipationCard({
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      await fetch(
        `/api/jam/projects/${projectId}/programs/${programParticipation.program.id}`,
        {
          method: 'PATCH',
        ...
        {completed && programParticipation.completedAt && (
          <div className="bg-muted p-3 rounded-md">
            <p className="text-xs font-medium">
              Completed on{' '}
              {new Date(programParticipation.completedAt).toLocaleDateString()}
            </p>
          </div>
        )}
      </CardContent>
...
export async function GET(
  ...
  let query = db
    ...
    .where(eq(programProjects.projectId, params.id));
...
export async function POST(
  ...
  if (!isAdmin.length) {
    return NextResponse.json(
      { error: 'Only project admins can join programs' },
      { status: 403 }
    );
  }
  ...
  const [newParticipation] = await db.insert(programProjects).values({
    projectId: params.id,
    programId,
    status: 'ACTIVE',
    joinedAt: new Date(),
