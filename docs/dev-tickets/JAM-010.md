    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Tell them why you'd like to work together
          </DialogDescription>
        </DialogHeader>
...
export async function getMentorRecommendations(userId: string) {
  ...
  const mentors = await db.select()
    ...
    .where(
      and(
        eq(usersTable.isMentor, true),
        eq(usersTable.mentorAvailability, 'available')
      )
    );
...
function calculateMatchScore(participant, mentor) {
  ...
  // Expertise match (25%)
  const expertiseMatch = mentor.expertiseAreas
    .filter(area => participant.interests?.includes(area))
    .length;
